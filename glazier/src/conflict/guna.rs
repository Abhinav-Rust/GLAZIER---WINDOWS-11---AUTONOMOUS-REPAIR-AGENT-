...
impl GunaClassifier {
    ...
    pub fn classify_state(observations: &[crate::memory::working::TimestampedObservation]) -> GunaType {
        ...
        for obs in observations {
            match &obs.observation {
                ObservationData::DeviceState { status, error_code, .. } => {
                    if status.to_lowercase().contains("error") || error_code.is_some() {
                        has_conflict = true; // Rajas (active conflict / error)
                    } else if status.to_lowercase() == "disabled" || status.to_lowercase() == "missing" {
