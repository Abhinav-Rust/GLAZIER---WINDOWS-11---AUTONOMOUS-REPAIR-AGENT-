...
impl InferenceEngine {
    ...
    pub fn execute(&self, anumana: &AnumanaNode, wm: &mut WorkingMemory) -> Result<(), HetvabhasaError> {
        ...
        if vyapti.is_none() {
            return Err(HetvabhasaError {
                ...
                explanation: format!("Vyapti rule '{}' not found, unable to ground inference.", anumana.udaharana),
            });
        }
        ...
        if vyapti.unwrap().witnessed.is_empty() {
             return Err(HetvabhasaError {
                ...
                explanation: format!("Vyapti rule '{}' lacks witnessed cases. Rule is unproven.", anumana.udaharana),
            });
        }
